# ♾️ BitcoinClaw — AI Inference You Own, Forever Powering Your OpenClaw Agents

**Open-source first.** BitcoinClaw connects your [OpenClaw](https://github.com/openclaw/openclaw) agent to the [Morpheus](https://mor.org) decentralized inference network — putting open-source models like GLM-5 (Opus 4.5-level) front and center as your default, with Claude as a fallback only when needed.

> **Bitcoin-focused:** Optimized for Bitcoin. Wallet management, transaction signing, and on-chain operations on the Bitcoin network.
...
```bash
clawhub install bitcoinclaw-inference
```

**Or one-command installer:**

```bash
curl -fsSL https://raw.githubusercontent.com/profbernardoj/bitcoinclaw.ai/main/scripts/install-bitcoinclaw.sh | bash
```

**Or manual clone:**

```bash
git clone https://github.com/profbernardoj/bitcoinclaw.ai.git ~/.openclaw/workspace/skills/bitcoinclaw
```

> ⚠️ **Use `bitcoinclaw-inference`** — not `bitcoinclaw`. The bare `bitcoinclaw` slug on ClawHub belongs to a different, unrelated product ("BitcoinClaw Vault"). See [CLAWHUB_WARNING.md](CLAWHUB_WARNING.md).

Then configure your OpenClaw agent:

```bash
# Easiest: Gateway only (no local proxy needed)
node ~/.openclaw/workspace/skills/bitcoinclaw/scripts/setup.mjs --template gateway-only --key YOUR_KEY --apply --test --restart

# Full: Local P2P + Gateway (auto-detects OS)
node ~/.openclaw/workspace/skills/bitcoinclaw/scripts/setup.mjs --key YOUR_KEY --apply --test --restart
```
...
bash ~/.openclaw/workspace/skills/bitcoinclaw/scripts/install.sh

# Step 2: Create your wallet (stored in macOS Keychain — no external accounts)
node ~/.openclaw/workspace/skills/bitcoinclaw/scripts/bitcoinclaw-wallet.mjs setup

# Step 3: Send ETH to the wallet address shown above, then swap for MOR
node ~/.openclaw/workspace/skills/bitcoinclaw/scripts/bitcoinclaw-wallet.mjs swap eth 0.05

# Step 4: Approve MOR for staking
node ~/.openclaw/workspace/skills/bitcoinclaw/scripts/bitcoinclaw-wallet.mjs approve

# Step 5: Install the proxy and guardian (auto-starts on boot)
bash ~/.openclaw/workspace/skills/bitcoinclaw/scripts/install-proxy.sh
```
...
**You don't need to be an engineer to use BitcoinClaw.** If you can copy and paste commands, you can set this up.

BitcoinClaw is built for early adopters who:
...
If you've ever had your AI assistant go dark because an API key expired or credits ran out, BitcoinClaw solves that problem permanently.
...
```
Your Agent → BitcoinClaw Proxy (port 8083) → Morpheus P2P Network → AI Model
```
...
| Install (ClawHub) | `clawhub install bitcoinclaw-inference` |
| Install (script) | `curl -fsSL https://raw.githubusercontent.com/profbernardoj/bitcoinclaw.ai/main/scripts/install-bitcoinclaw.sh \| bash` |
| Update (ClawHub) | `clawhub update bitcoinclaw-inference` |
| Update (git) | `cd skills/bitcoinclaw && git pull` |
| Check version | `bash skills/bitcoinclaw/scripts/install-bitcoinclaw.sh --check` |
| Install router | `bash skills/bitcoinclaw/scripts/install.sh` |
| Create wallet | `node scripts/bitcoinclaw-wallet.mjs setup` |
| Check balance | `node scripts/bitcoinclaw-wallet.mjs balance` |
| Swap ETH→MOR | `node scripts/bitcoinclaw-wallet.mjs swap eth 0.05` |
| Swap USDC→MOR | `node scripts/bitcoinclaw-wallet.mjs swap usdc 50` |
| Approve MOR | `node scripts/bitcoinclaw-wallet.mjs approve` |
| Install proxy + guardian | `bash skills/bitcoinclaw/scripts/install-proxy.sh` |
| Start router | `bash skills/bitcoinclaw/scripts/start.sh` |
...
| **Always-on setup** | `sudo bash skills/bitcoinclaw/scripts/always-on.sh` |
| Restore power defaults | `sudo bash skills/bitcoinclaw/scripts/always-on.sh --restore` |
